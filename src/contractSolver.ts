...
export async function main(ns: NS) {
	...
	const input = JSON.parse(ns.args[0] as string);
	const k = input[0];
	const data = input[1];
	ns.print(k);
	ns.print(data);
	...
	for (let i = 0; i < data.length; i++) {
		if (i == data.length) continue; //done
		if (i > 0 && data[i] > data[i - 1]) continue; //found a 'starting' number smaller than the previous
		let peak = data[i + 1];
		for (let j = i + 1; j < data.length; j++) {
			ns.print(`${i} 1 ${j}`);
			while (j < data.length && data[j + 1] >= data[j]) j++;
			ns.print(`${i} 2 ${j}`);
	...
	for (let i = 0; i < rangeProfits.length; i++) {
		...
		if (rangeProfits[i][2] + cost < 0) addit = false; // useless parent, doesn't provide profit at all
		if (addit) {
